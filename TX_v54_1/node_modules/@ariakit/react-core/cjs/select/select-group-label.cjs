"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _DGL5AMAWcjs = require('../__chunks/DGL5AMAW.cjs');
require('../__chunks/TDUVWW2F.cjs');
require('../__chunks/K56WQU76.cjs');




var _Z5PUIG6Acjs = require('../__chunks/Z5PUIG6A.cjs');
require('../__chunks/5WLTMVVQ.cjs');
require('../__chunks/7WUTS26Y.cjs');
require('../__chunks/72I2GWXF.cjs');

// src/select/select-group-label.ts
var useSelectGroupLabel = _Z5PUIG6Acjs.createHook.call(void 0, 
  (props) => {
    props = _DGL5AMAWcjs.useCompositeGroupLabel.call(void 0, props);
    return props;
  }
);
var SelectGroupLabel = _Z5PUIG6Acjs.createComponent.call(void 0, 
  (props) => {
    const htmlProps = useSelectGroupLabel(props);
    return _Z5PUIG6Acjs.createElement.call(void 0, "div", htmlProps);
  }
);
if (process.env.NODE_ENV !== "production") {
  SelectGroupLabel.displayName = "SelectGroupLabel";
}



exports.SelectGroupLabel = SelectGroupLabel; exports.useSelectGroupLabel = useSelectGroupLabel;
